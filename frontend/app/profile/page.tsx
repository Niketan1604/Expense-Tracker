...
export default function ProfilePage() {
  ...
  useEffect(() => {
    if (profile) {
      const n = profile.name || authUser?.username || ''
      const c = profile.currency || 'INR'
      setName(n);         setOriginalName(n)
      setCurrency(c);     setOriginalCurrency(c)
    } else if (!loading && authUser) {
      const n = authUser.username || ''
      setName(n); setOriginalName(n)
    }
    ...
    <div className="page animate-fadeUp pb-24 lg:pb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="card p-7 rounded-2xl flex flex-col items-center text-center">
          ...
          <p className="text-lg font-black" style={{ color: 'var(--text)', letterSpacing: '-0.02em' }}>
            {profile?.name || name || '—'}
          </p>
          ...
          <div className="w-full mt-4 pt-4" style={{ borderTop: '1px solid var(--border)' }}>
            <button onClick={toggle}
              ...
              onMouseEnter={e => (e.currentTarget.style.background = 'var(--mint-dim)')}
              onMouseLeave={e => (e.currentTarget.style.background = 'var(--surface-2)')}>
              <span style={{ color: 'var(--muted)' }}>Appearance</span>
              <div className="flex items-center gap-2">
                <span className="text-xs" style={{ color: 'var(--muted)' }}>{isDark ? 'Dark' : 'Light'}</span>
                {isDark
        ...
        <div className="md:col-span-2 card p-7 rounded-2xl">
          <h3 className="font-black text-base mb-6" style={{ color: 'var(--text)', letterSpacing: '-0.02em' }}>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <input type="text" value={name} onChange={e => setName(e.target.value)} required
                className="fm-input rounded-xl" placeholder="Your name" />
            </div>
            ...
            <div>
              <label className="block text-xs font-bold uppercase tracking-widest mb-1.5"
                style={{ color: 'var(--muted)' }}>Currency</label>
